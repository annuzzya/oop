    ...
    private async void OnLoadXmlClicked(object sender, EventArgs e)
{
    try
    {
        ...
        if (result != null)
        {
            if (!result.FileName.ToLower().EndsWith(".xml"))
            {
                await DisplayAlert("Помилка", "Оберіть файл із розширенням .xml", "OK");
                return;
            }
...
private async void OnLoadXslClicked(object sender, EventArgs e)
{
    try
    {
        var fileTypes = new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>>
        {
            ...
            { DevicePlatform.Android, new[] { "text/xsl" } },
            { DevicePlatform.WinUI, new[] { ".xsl" } }
        });
        ...
        if (result != null)
        {
            if (!result.FileName.ToLower().EndsWith(".xsl"))
            {
                await DisplayAlert("Помилка", "Оберіть файл із розширенням .xsl", "OK");
                return;
            }
